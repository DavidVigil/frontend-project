...
import {
  ...
  Toolbar,
  Typography,
} from "@mui/material"; // Components from material UI
...
export default function AppBarGlobal() {
  ...
  const handleChange = (newValue) => {
    // Function to handle changes (clics on tabs)
    setValue(newValue);
  };
  ...
  const goToMyApps = () => {
    // To go to my apps page
    setValue(1); // To highlight this tab
    router.push("/myApps");
  };

  const handleSignOut = () => {
    // Function to exit the session when you are inside
    ...
    >
      <Toolbar sx={{ minHeight: "64px" }}>
        ...
        <Box
          ...
          onClick={goToHome}
        >
          <Image src="/logo.png" alt="FI Marketplace" width={40} height={40} />
          <Typography
            variant="h5"
            sx={{
        ...
        <Tabs
          ...
          sx={{
            ...
            "& .MuiTabs-indicator": {
              backgroundColor:
                value === false ? "transparent" : theme.palette.secondary.main, // Yellow highlight unless on Home
            },
          }}
        >
          ...
          <Tab
            label="About"
            onClick={goToAbout}
            sx={{ color: value === 0 ? theme.palette.text.light : "#ffffff" }}
          />
          <Tab
            label="My Apps"
            onClick={goToMyApps}
            sx={{ color: value === 1 ? theme.palette.text.light : "#ffffff" }}
          />
        </Tabs>
        {isAuthenticated ? ( // Are we in or out?
          <Button color="secondary" onClick={handleSignOut} variant="outlined">
            Sign Out
          </Button> // If we're in
        ) : (
          // If wer'e out:
